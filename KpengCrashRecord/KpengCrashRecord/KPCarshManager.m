...
#import "KPCarshManager.h"
#include <libkern/OSAtomic.h>
#include <execinfo.h>
#import "KPCarshLog.h"
#import "AppDelegate.h"
@interface KPCarshManager ()
...
- (void)handleException:(NSException *)exception{
    ...
    [KPCarshLog collectCrashInfoWithException:exception exceptionStackInfo:stackInfo viewControllerStackInfo:[self getCurrentViewControllerStackInfo]];
...
void MyExceptionHandler(NSException *exception) {
    ...
    [[[KPCarshManager alloc] init] performSelectorOnMainThread:@selector(handleException:) withObject:[NSException exceptionWithName:[exception name] reason:[exception reason] userInfo:userInfo] waitUntilDone:YES];
...
void SignalHandler(int signal) {
    ...
    NSArray *callBack = [KPCarshManager backtrace];
    ...
    [[[KPCarshManager alloc] init] performSelectorOnMainThread:@selector(handleException:) withObject:signalException waitUntilDone:YES];
}

static void MySignalHandler(int signal, siginfo_t* info, void* context) {
    SignalHandler(signal);
    
    // 处理前者注册的 handler 有可能集成了三方的信号处理 在这里
    //    if (signal == SIGABRT) {
